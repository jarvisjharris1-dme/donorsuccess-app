...
export async function updateUserGrantRoleAction(_prevState: ActionState, formData: FormData): Promise<ActionState> { const session=await auth();if(!session)redirect('/login');assertRole(session.user.role as Role,Role.ADMIN);const userId=formData.get('userId'),grantRole=formData.get('grantRole');if(typeof userId!=='string'||!userId)return{error:'Missing user.'};const value=typeof grantRole==='string'&&grantRole&&grantRole in GrantRole?grantRole as GrantRole:null;const db=forOrg(session.user.organizationId);await db.user.update({where:{id:userId},data:{grantRole:value}});revalidatePath('/settings'); }
export async function toggleUserActiveAction(_prevState: ActionState, formData: FormData): Promise<ActionState> { const session=await auth();if(!session)redirect('/login');assertRole(session.user.role as Role,Role.ADMIN);const userId=formData.get('userId');if(typeof userId!=='string'||!userId)return{error:'Missing user.'};if(userId===session.user.id)return{error:'You cannot deactivate your own account.'};const db=forOrg(session.user.organizationId),target=await db.user.findUniqueOrThrow({where:{id:userId}});if(target.role===Role.OWNER&&target.isActive){const owners=await db.user.count({where:{role:Role.OWNER,isActive:true}});if(owners<=1)return{error:'An organization must have at least one active Owner.'};}await db.user.update({where:{id:userId},data:{isActive:!target.isActive}});revalidatePath('/settings'); }
export async function changePasswordAction(_prevState: ActionState, formData: FormData): Promise<ActionState> { const session=await auth();if(!session)redirect('/login');const parsed=changePasswordSchema.safeParse({currentPassword:formData.get('currentPassword'),newPassword:formData.get('newPassword'),confirmPassword:formData.get('confirmPassword')});if(!parsed.success)return{error:parsed.error.issues[0]?.message??'Check your password.'};const user=await prisma.user.findUnique({where:{id:session.user.id}});if(!user?.passwordHash)return{error:'Password login is not enabled for this account.'};const valid=await bcrypt.compare(parsed.data.currentPassword,user.passwordHash);if(!valid)return{error:'Current password is incorrect.'};const passwordHash=await bcrypt.hash(parsed.data.newPassword,12);await prisma.user.update({where:{id:user.id},data:{passwordHash}});return{success:'Password updated.'}; }
