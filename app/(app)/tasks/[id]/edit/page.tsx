...
export default async function EditTaskPage({ params }: { params: { id: string } }) {
  ...
  const [task, users, donors, opportunities, grantOpportunities] = await Promise.all([
    db.task.findUnique({ where: { id: params.id } }),
    db.user.findMany({
      where: { isActive: true },
  ...
  return (
    <div className="max-w-2xl">
      <h1 className="text-2xl font-extrabold text-gray-900">Edit task</h1>
      <p className="mt-1 text-sm text-gray-600">Update this task&rsquo;s details.</p>
