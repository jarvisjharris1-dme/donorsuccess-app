import { redirect } from 'next/navigation';
import { auth } from '@/auth';
import { forOrg } from '@/lib/tenant-db';
import { permissions } from '@/lib/permissions';
import { Role } from '@prisma/client';
import { donorDisplayName } from '@/lib/format';
import TaskForm from '@/components/tasks/TaskForm';

export default async function NewTaskPage() {
  const session = await auth();
  if (!permissions.canEditDonors(session!.user.role as Role)) {
    redirect('/tasks');
  }

  const db = forOrg(session!.user.organizationId);

  const [users, donors, opportunities, grantOpportunities] = await Promise.all([
    db.user.findMany({
      where: { isActive: true },
      select: { id: true, name: true, email: true },
      orderBy: { name: 'asc' },
    }),
    db.donor.findMany({
      select: { id: true, firstName: true, lastName: true, organizationName: true },
      orderBy: { lastName: 'asc' },
    }),
    db.opportunity.findMany({
      select: { id: true, name: true },
      orderBy: { name: 'asc' },
    }),
    db.grantOpportunity.findMany({
      select: { id: true, name: true },
      orderBy: { name: 'asc' },
    }),
  ]);

  const donorOptions = donors.map((d) => ({ id: d.id, name: donorDisplayName(d) }));

  return (
    <div className="max-w-2xl">
      <h1 className="text-2xl font-extrabold text-gray-900">New task</h1>
      <p className="mt-1 text-sm text-gray-600">Add a to-do for yourself or your team.</p>

      <div className="mt-6">
        <TaskForm
          users={users}
          donors={donorOptions}
          opportunities={opportunities}
          grants={grantOpportunities}
          currentUserId={session!.user.id}
        />
      </div>
    </div>
  );
}
